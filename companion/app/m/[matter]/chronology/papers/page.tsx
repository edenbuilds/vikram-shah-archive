...
export default async function PaperDates({ params, searchParams }: { params: Promise<{ matter: string }>; searchParams: Promise<{ y?: string; q?: string; o?: string }> }) {
  const { matter } = await params;
  const sp = await searchParams;
  const { supabase, user } = await requireUser();
  await getMatter(supabase, matter);
  const [saved, now, pins, { data: docs }, { data: mine }, printed] = await Promise.all([
  ...
  const href = (o: Record<string, string>) => `?${new URLSearchParams({ ...(q ? { q } : {}), ...(year ? { y: year } : {}), ...(newest ? { o: "newest" } : {}), ...o })}`;
  ...
  return (
    <div className="stack">
      ...
      <form className="row" style={{ alignItems: "end" }}>
        ...
        {newest && <input type="hidden" name="o" value="newest" />}
        <button className="btn ghost" style={{ flex: "0 0 auto" }}>Find</button>
      </form>
      <div className="chips">
        <Link className={`chip${!year ? " on" : ""}`} href={href({ y: "" })}>All years ({data.dates.length})</Link>
        {years.map((y) => <Link key={y} className={`chip${year === y ? " on" : ""}`} href={href({ y })}>{y} ({count(y)})</Link>)}
      </div>
      <div className="chips" aria-label="Order">
        <Link className={`chip${!newest ? " on" : ""}`} href={href({ o: "" })}>Oldest first</Link>
        <Link className={`chip${newest ? " on" : ""}`} href={href({ o: "newest" })}>Newest first</Link>
      </div>
              ...
              {d.mentions.filter((x) => !q || x.quote.toLowerCase().includes(q)).map((x, i) => (
                <div key={i} className="mention">
                  <Link href={`/m/${matter}/d/${x.doc}?p=${x.page}`} className="mention-text">
                    ...
                    <cite>{title.get(x.doc) ?? x.doc}, {pageLabel(x.page, printed[x.doc]?.[x.page])} →</cite>
                  </Link>
