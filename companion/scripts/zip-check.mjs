...
if (file && file !== "-") await p.selectOption(".side select", file);
const [dl] = await Promise.all([p.waitForEvent("download", { timeout: 600000 }), p.click("text=Download everything (.zip)")]);
await dl.saveAs(`${out}/${dl.suggestedFilename()}`);
console.log("saved", dl.suggestedFilename(), "|", await p.textContent(".side .subtle"));
await b.close();
